...
export default {
  ...
  <div v-if="show" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="$emit('close')">
    <div class="bg-ink2 text-paper rounded-2xl shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden">
      ...
      <div class="flex-grow grid grid-cols-1 md:grid-cols-12 overflow-hidden divide-y md:divide-y-0 md:divide-x divide-black/10">
        <div class="md:col-span-7 p-4 overflow-y-auto bg-paperDim">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <button v-for="m in visibleMenu" :key="m.id" @click="$emit('add-item', m)"
              class="bg-white rounded-xl p-2.5 text-left border border-black/10 hover:border-copper transition-all">
        ...
        <div class="md:col-span-5 p-4 flex flex-col justify-between overflow-y-auto">
          <div>
            ...
            <div class="space-y-1.5 mb-3 max-h-48 overflow-y-auto pr-1">
              ...
              <div v-for="it in activeTable?.items" :key="it.uid" class="flex items-center justify-between text-sm bg-paperDim rounded-lg px-2.5 py-1.5">
                ...
                <div class="flex items-center gap-1.5 font-mono">
                  <button @click="$emit('change-qty', it, -1)" class="w-5 h-5 rounded bg-black/10 hover:bg-black/20 text-xs">−</button>
            ...
            <div class="bg-copper/10 border border-copper/25 rounded-xl p-3 space-y-1.5 text-sm">
              <div class="flex justify-between text-ash"><span>Tổng tiền order</span><span class="font-mono font-semibold text-paper">{{ fmt(orderTotal) }}</span></div>
          ...
          <div class="pt-4 space-y-2 border-t border-black/10 mt-3">
            <div v-if="orderRemaining > 0" class="flex gap-2">
              <input :value="partialPayInput" @input="$emit('update:partialPayInput', $event.target.value === '' ? null : Number($event.target.value))"
                type="number" placeholder="Số tiền khách trả" class="flex-grow bg-white text-paper border border-black/15 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:border-copper">
              <button @click="$emit('partial-pay')" class="bg-copper hover:bg-copperDk text-ink font-semibold px-3 rounded-lg text-sm shrink-0">Ghi nhận trả</button>
            </div>
            <button @click="$emit('save-temp')" class="w-full bg-white text-paper border border-black/15 hover:border-black/30 font-medium py-2.5 rounded-xl text-sm flex items-center justify-center gap-2">
